...
(gdb) disas main
   ...
   ---> Align stack to 16 bytes (ABI).

   0x0804887f <+6>:     sub    $0x50,%esp
   ---> Allocate locals: login buffer at esp+0x2c, serial at esp+0x28, canary slot at esp+0x4c.

   0x08048882 <+9>:     mov    0xc(%ebp),%eax
   0x08048885 <+12>:    mov    %eax,0x1c(%esp)
   ---> Save **argv** pointer on stack (used later only if needed; here mainly layout).

   0x08048889 <+16>:    mov    %gs:0x14,%eax
   0x0804888f <+22>:    mov    %eax,0x4c(%esp)
   0x08048893 <+26>:    xor    %eax,%eax
   ---> Load stack canary from TLS and store copy before body.
   ...
   ---> **fgets(login, 0x20, stdin)** — up to 32 bytes into buffer at esp+0x2c.
   ...
   ---> Prompt for serial number (**%u** style).

   0x0804891d <+164>:   mov    $0x8048a60,%eax
   ---> Address of **"%u"** scanf format.
   ...
   ---> **scanf("%u", &serial)** into dword at esp+0x28.
   ...
   ---> **auth(login_buf, serial)** — second arg is unsigned int.

   0x08048946 <+205>:   test   %eax,%eax
   0x08048948 <+207>:   jne    0x8048969 <main+240>
   ---> Branch if auth returned **non-zero** (failure).
   ...
   ---> Verify stack canary before exit.
...
(gdb) disas auth
   ...
   ---> Prologue; locals: len at -0xc, accumulator at -0x10, loop index at -0x14.

   0x0804874e <+6>:     movl   $0x8048a63,0x4(%esp)
   ---> Second arg to strcspn: charset string (newline etc.).

   0x08048756 <+14>:    mov    0x8(%ebp),%eax
   0x08048759 <+17>:    mov    %eax,(%esp)
   0x0804875c <+20>:    call   0x8048520 <strcspn@plt>
   ---> **strcspn(login, "\n")** — find first unwanted char index.

   0x08048761 <+25>:    add    0x8(%ebp),%eax
   0x08048764 <+28>:    movb   $0x0,(%eax)
   ---> NUL-terminate login at first delimiter (strip newline).
   ...
   ---> **strnlen(login, 32)** — bounded length.
   ...
   ... pop / stub
   ---> Alignment / stack protector micro-stub around this point.
   ...
   ---> If **len ≤ 5**, return **1** (failure) — login too short.
   ...
   ---> **ptrace(PTRACE_TRACEME, …)** — detect debugger; if already traced, ptrace fails.

   0x080487ba <+114>:   cmp    $0xffffffff,%eax
   0x080487bd <+117>:   jne    0x80487ed <auth+165>
   ---> If ptrace **≠ -1**, continue to hash; else “anti-debug” path.

   0x080487bf <+119>:   movl   $0x8048a68,(%esp)
   ... puts ×3
   ---> Print “TAMPERING DETECTED” style messages.

   0x080487e3 <+155>:   mov    $0x1,%eax
   0x080487e8 <+160>:   jmp    0x8048877 <auth+303>
   ---> Return **1** when ptrace failed (debugger present).
   ...
   ---> Seed accumulator: **(login[3] ^ 0x1337) + 0x5eeded** (32-bit).
   ...
   ---> If **login[i] ≤ 0x1f** (non-printable), return **1**.

   0x08048823 <+219>:   mov    -0x14(%ebp),%eax
   ... load byte, sign-extend
   0x08048831 <+233>:   mov    %eax,%ecx
   0x08048833 <+235>:   xor    -0x10(%ebp),%ecx
   ---> **c = login[i] ^ acc** (32-bit XOR).
   ...
   ---> **acc += hash_mix(c)** — signed 32-bit magic multiply / shift (obfuscated mixing).
   ...
   ---> Compare **serial** (second parameter) with **acc**; **0** = success, **1** = fail.
...
**Exploitation note:** Serial must match the deterministic hash of the login; **ptrace** check can be bypassed in a debugger by patching or preloading depending on your lab rules. **fgets** length and **auth** length check constrain the login string.
